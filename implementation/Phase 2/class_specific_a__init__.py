...
class SS3:
    ...
    __l__ = .5
    __p__ = 1
    __a__ = [0,0,0] # Change by NLP Lab - As many entries as classes in dataset
    ...
    def __classify_ngram__(self, ngram):
        ...
        # Changed by NLP Lab - to apply threshold to each cv individually per class
        temp = []
    ...
    def __classify_sentence_test__(self, sent, prep, json=False, prep_func=None):
        ...
        while wcur < sent_len:
            ...
            # Changed by NLP Lab
            if (max_gv > self.__alpha__[self.get_category_name(cats_max_cv.index(max_gv))]):
                ...
                max_gv_sum_1_grams = max([
                    sum([(categories[ic][VOCAB][wi][CV] if wi in categories[ic][VOCAB] else 0) for wi in cats_ngrams_iword[ic]]) 
                    for ic in cats ])
    ...
    def __classify_sentence__(self, sent, prep, json=False, prep_func=None):
        ...
        while wcur < sent_len:
            ...
            use_ngram = True
            # Changed by NLP Lab
            if (max_gv > self.__a__[cats_max_cv.index(max_gv)]):
                icat_max_gv = cats_max_cv.index(max_gv)
                ngram_max_gv = cats_ngrams_cv[icat_max_gv].index(max_gv)
                offset_max_gv = cats_ngrams_offset[icat_max_gv][ngram_max_gv] + 1
    ...
    def set_a(self, value):
        ...
        :type value: float
        """
        temp = list(self.__a__)
        for i, v in enumerate(value):
            temp[i] = v
        self.__a__ = tuple(temp)
    ...
    def predict(
        ...
        if self.get_ngrams_length() == 2 and self.__summary_ops_are_pristine__():
